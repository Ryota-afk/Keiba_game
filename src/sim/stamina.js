 ...
 * ⚠️**この「短距離まで届く」問題は、柔軟性の幅の式を動かしても直らない**（実測）。
 * 幅の式は別の理由（帯が実際の勝ち鞍の幅より広すぎる）で狭めた——`aptitudeWidth`を参照。
 ...
 * ⚠️⚠️**必要以上に広げないこと**（2026-09-07にユーザーが指摘）。`400 + 1200×柔軟性`だった
 * ときは、⭐**ダービー馬51頭の適正距離が実際の勝ち鞍の幅より中央値+300m・最大+900m広く**、
 * 2000〜2400mしか勝っていない馬に1400〜2700mを与えていた。この式なら最大+300mに収まる。
 * ⭐**柔軟性の記号自体も実際の勝ち鞍の幅で補正する**（`arch/historical-horses.md`§2）。
 */
export function aptitudeWidth(horse) {
  return 300 + normalizedAbilities(horse).flexibility * 1000; // 300〜1300m
}
