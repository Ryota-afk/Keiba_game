...
const APT_A = 0.5;
/** 適正距離より短い方向の幅の倍率。
 * ⚠️⚠️**1頭に合わせて決めないこと。** 2026-09-07に一度「ディープインパクトの下端が
 * ちょうど2000mになる」という理由で1.1にしたが、⭐**51頭の勝ち鞍303鞍で測り直したところ
 * 適正帯に入る割合が89.1%→85.5%へ下がった**（改悪だった）。1.6は同じ指標が頭打ちになる点
 * （1.1で85.5%・1.6で89.1%・2.6で90.1%）。手順は`arch/historical-horses.md`§2。 */
const APT_UNDER_SCALE = 1.6;
...
export function optimalDistance(horse) {
  // ⚠️⚠️`900 + 18×スタミナ`は**実測で較正した値**。旧`1000 + 20×スタミナ`は最初に置いた
  // 仮の値で、一度も戦績と突き合わせておらず、⭐**ダービー馬51頭中51頭で実際の重心より
  // 長く出ていた**（中央値+202m）。較正が有効なのはスタミナ47〜81の範囲だけ
  // （`arch/historical-horses.md`§3の外挿の警告）。
  const raw = 900 + normalizedAbilities(horse).stamina * 1800; // 900〜2700m
