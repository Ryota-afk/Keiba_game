...
// 画面に出す勝ちタイムの安全網。⚠️**普段は働かない**——タイムを決めるのは
// `sim/raceSim.js`の基準速度のほう。⚠️2026-09-16まで141.0〜148.0秒で、実測すると
// 120通り中35通りが上限に当たっていた（＝2:28.0という同じ値が出ていた）。
// 今のsimの実測は最速2:25.8〜最遅2:28.6（同120通り）なので、その外側に置き直した。
const GOAL_TIME_MIN = 140.0; // 2:20.0
const GOAL_TIME_MAX = 151.0; // 2:31.0
...
export function startDreamDerbySim(saveSeed, entries) {
  ...
  return runRaceSim({
    ...
    // ⚠️**基準速度の倍率も出走馬から計算させない**（2026-09-16）。
    // 普段のレースは「出走馬の平均能力」で勝ちタイムを決めるが、その式は架空馬の分布
    // （平均0.48・重賞0.52）で較正してある。史実のダービー馬18頭は平均0.75で、
    // そのまま当てると倍率1.10＝2分18秒8になり、実際の日本ダービーの最速記録より
    // 1.8秒速い値が出た（実測）。⭐史実馬の能力値は架空馬と別の物差しなので、
    // ここは1.00＝「史実の重賞の基準タイムで走る」に固定する。`devlog/wave08.md`§6。
    fieldFactor: 1,
  });
}
